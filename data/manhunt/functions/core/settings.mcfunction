...
data modify storage manhunt:settings mmh.mechanics.setup set value {"hunted_size":1s}

# The Y coordinate can be anything but is included because it minimizes calls to NBT.
data modify storage manhunt:settings mmh.mechanics.worldborder.set set value {"setup":{"size":30},"game":{"size":6000000,"center":[0d,0d,0d]}}
